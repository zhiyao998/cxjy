			   ...
			   function(data){
					...
					if(data.length > 0){
						if(data[0].resultType == 1){
							...
							json += "<tbody>";
							for(var i = 0; i < data.length; i++){
								json += "<tr><td>" + data[i].accessoryExamOrder + "</td><td>" + data[i].accessoryExamName +"</td><td><input disabled='true' class='easyui-textbox' value='" + data[i].accessoryExamResult + "'></input></td><td>wwwww</td><td>qqqqqqq</td>" + 
							...
							var json = "<form id='upload' enctype='multipart/form-data' method='post' >" + 
										...
										"<div><label>影像检查</label></div><div><label>" + data[0].accessoryExamName + ":</label><br><input name='score' class='easyui-numberbox' value='0'>" + 
...
function submitAccessoryExam() {
	...
	if(trs != null){
		...
		for(var i = 0; i < trs.length - 1; i++){
			...
			var input1 = $(td[2]).children("input");
			var input2 = $(td[5]).children("input");
			var input3 = $(td[6]).children("input");
			if(flag){
				json += "{\"accessoryExamOrder\":\""+ $(td[0]).text() +"\",\"accessoryExamName\":\""+ $(td[1]).text() + "\",\"accessoryExamResult\":\"" + $(input1).val()
					+ "\",\"score\":\""+ $(input2).val() + "\",\"resultType\":\""+ $(input3).val()
